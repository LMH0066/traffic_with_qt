...
// resize short within
inline cv::Mat ResizeShortWithin(cv::Mat src, int short_size, int max_size, int mult_base, double &scale) {
    double h = src.rows;
    double w = src.cols;
    ...
    scale = static_cast<double>(short_size) / static_cast<double>(im_size_min);
...
inline cv::Mat PlotBbox(cv::Mat img, NDArray bboxes, NDArray scores, NDArray labels,
                        ...
                        std::map<int, cv::Scalar> colors, bool verbose, double scale,
                        cv::Mat frames[8], int frame_number, int top_rows, int* nums,
                        bool empty_tracking) {
//    int nums[8] = {0};
...
//     labels.WaitToRead();
//    clock_t t_r2 = clock();
//    std::cout<<"time_read:"<<(t_r2 - t_r1) * 1.0 / CLOCKS_PER_SEC<<std::endl;
//    if (verbose) {
...
//    }
    ...
    txtOutput << record_number << ".jpg " << number << endl;
    record_number++;
    ...
    for (int i = 0; i < num; ++i) {
        ...
        if (colors.find(cls_id) == colors.end()) {
            colors[cls_id] = HSV2BGR(cv::Scalar(255, 0.75, 0.95));
            //     create a new color
        ...
        cv::Point pt1(bboxes.At(0, i, 0) / scale, bboxes.At(0, i, 1) / scale);
        cv::Point pt2(bboxes.At(0, i, 2) / scale, bboxes.At(0, i, 3) / scale);
        cv::rectangle(img, pt1, pt2, color, 2);

        txtOutput << bboxes.At(0, i, 0) / scale << " " << bboxes.At(0, i, 1) / scale << " "
                  << bboxes.At(0, i, 2) / scale << " " << bboxes.At(0, i, 3) / scale << endl;

        cv::Point pt = (pt1 + pt2) / 2;
        bool find = false;
        int now_cols = 0, j = 0;
        if (frame_number == 1) {
            nums[0]++;
            ...
            if (pt.y > top_rows)
                j += frame_number/2;
            for (int k = 0; k < frame_number/2 && !find; k++) {
                if (now_cols < pt.x && frames[j + k].cols + now_cols > pt.x)
                    nums[j + k]++;
                now_cols += frames[j + k].cols;
            }
        }
